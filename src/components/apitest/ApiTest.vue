      ...
      <el-collapse-item  name="2" class="collapse-class">
            ...
            <!--接口参数列表-->
                    ...
                    <tr v-for="(key,index) in tree.parameters">
                        ...
                        <td ><el-input v-model="key.name" readonly></el-input></td>
                        <td><el-input :placeholder="key.description" v-model="key.value"></el-input></td>
                        <td><el-input  v-model="key.required" readonly></el-input></td>
                        <td><el-button size="small" type="danger" @click="delParam(index)" v-show="key.required===false">删除</el-button></td>
...
export default {
  ...
  methods : {
      ...
      getSendParam(){
        ...
        if(v_this.tree.sendWay.toLowerCase()==='get'){
          if(tree.parameters){
            for(let i= 0 ; i< v_this.tree.parameters.length ; i++){
              if(tree.parameters[i].checked){
                url=url.replace('{'+tree.parameters[i].name+'}',tree.parameters[i].value)
